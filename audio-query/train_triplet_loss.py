...
RESULTS_DIRECTORY = 'triplet-results-audio-query'
...
def train_with_triplet_loss(epochs=EPOCHS, batch_size=1):
    ...
    with open(f"{RESULTS_DIRECTORY}/information.txt", "w") as file:
        ...
        # file.write("\n")
        file.write(f"Query is Audio. Vision+Tatcile as Retrieval. Classifiaction with vision. Training with margin {MARGIN} and {epochs} epochs, and batch size {batch_size}.")


    # Load your embeddings
    query_embeddings = np.load("/scratch/users/k21171248/c-entropy-results-audio-query/audio_embeddings_train.npy", allow_pickle=True).item()
    fused_embeddings = np.load("/scratch/users/k21171248/c-entropy-results-audio-query/visual_tactile_fused_train.npy", allow_pickle=True).item()
    query_embeddings_test = np.load("/scratch/users/k21171248/c-entropy-results-audio-query/audio_embeddings_test.npy", allow_pickle=True).item()  
    fused_embeddings_test = np.load("/scratch/users/k21171248/c-entropy-results-audio-query/visual_tactile_fused_test.npy", allow_pickle=True).item()  
