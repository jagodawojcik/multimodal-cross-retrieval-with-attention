...
RESULTS_DIRECTORY = 'c-entropy-results-audio-query'
...
def train_with_cross_entropy(epochs_pre = EPOCHS_PRETRAIN, epochs_cross_entropy=EPOCHS_C_ENTROPY, batch_size=BATCH_SIZE):
    ...
    # Save the model
    torch.save(tactile_network.state_dict(), f"{RESULTS_DIRECTORY}/model_pretrain.pth")
    ...
    for epoch in range(epochs_cross_entropy):
        ...
        print(f'Epoch {epoch}, Train Loss: {epoch_train_loss}, Test Loss: {test_loss}, Audio Test Loss: {vision_test_loss}')  # <- Update the print statement
    ...
    np.save(f"{RESULTS_DIRECTORY}/visual_tactile_fused_train.npy", dict(visual_tactile_fused_train))
    ...
    np.save(f"{RESULTS_DIRECTORY}/visual_tactile_fused_test.npy", dict(visual_tactile_fused_test))
    ...
    loss_dict = {'train_losses': train_losses, 'test_losses': test_losses, 'test_audio_losses': test_losses_vision}
